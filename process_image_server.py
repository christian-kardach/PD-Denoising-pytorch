...
def main(input_file, opt):
    ...
    print("info:Starting to process: {input_file} of type {image_type}".format(input_file=input_file,
    ...
    if opt.new_slicer:
        ...
        for patch in zl.patches:
            ...
            print("progress:{patches}|{total}".format(patches=patches_done, total=zl.patch_count))
            patches_done += 1
    ...
    print("download:{session}/{filename}|{original_name}".format(session=opt.session,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--originalName", type=str, help='')
    parser.add_argument("--session", type=str, help='')
    args = parser.parse_args()

    Opt.wbin = args.wbin
    Opt.patch_margin = args.pmargin
    Opt.file_path = args.filepath
    Opt.original_name = args.originalName
    Opt.session = args.session

    # Check images
    print('info:Checking image types...')
    is_grayscale = utils.isgray(args.filepath)
    if is_grayscale:
        Opt.color = 0
    else:
        Opt.color = 1

    main(args.filepath, Opt)
